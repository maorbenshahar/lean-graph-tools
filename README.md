...
Finds which declarations transitively depend on `sorry` and renders the dependency graph.
...
```bash
# Single declaration
sorry-graph my_theorem

# All sorry in a file/module
sorry-graph -f Path/To/Module

# Project-wide sorry summary
sorry-graph --all

# JSON output
sorry-graph my_theorem --json
```

Example output:
```
$ sorry-graph main_theorem
...
```bash
# Context for a file
sig-graph -f InfoTheory/Measurement/POVM

# Context for specific declarations
sig-graph myTheorem anotherDef

# Output to file
sig-graph -f MyModule -o context.lean

# JSON output
sig-graph -f MyModule --json
```

### upstream-graph / downstream-graph
...
upstream-graph MyProject.Top.mainTheorem

# All transitive downstream declarations
downstream-graph MyProject.Core.keyLemma

# Only direct dependencies/users
upstream-graph MyProject.Top.mainTheorem --direct
downstream-graph MyProject.Core.keyLemma --direct

# JSON output
upstream-graph MyProject.Top.mainTheorem --json
downstream-graph MyProject.Core.keyLemma --json
```
...
```bash
pip install -e /path/to/lean-graph-tools
```

The `sorry-graph`, `sig-graph`, `downstream-graph`, `upstream-graph`, and
