...
def _render_decl_source(decl: DeclInfo) -> str:
    ...
    best-effort stub when none was exported."""
    if decl.source_text:
        return _apply_elision(decl, "sorry")
    name = display_name(decl)
    sig = decl.type_signature or "_"
    if decl.kind in ("theorem", "lemma"):
        return f"-- (no source slice; synthesized)\ntheorem {name} : {sig} := by sorry"
    return f"-- (no source slice available for {decl.kind} {name})"
