/-
  Export all project declarations as JSON for the source-faithful audit digest.

  Usage:
    lake build  -- must build first
    lake env lean --run <path>/ExportSigs.lean <RootModule>

  Output: per declaration — name, kind, module, type-level dependencies, the
  verbatim `source_text` slice (with `byTactic_ranges` marking its proof blocks)
  and `decl_namespace`, structure field metadata (name/projName/fromParent),
  parent structures, docstring, and line; plus per-module import/open/notation/
  variable/context maps.

  Deliberately does NO pretty-printing or elaboration of signatures or values:
  the digest renders from the verbatim source slice, so the declaration's
  statement and body are shown exactly as authored. Earlier versions also emitted
  `ppExpr`-rendered `type_signature`/`value`/`fields[].type`/`constructors`; those
  ran a per-node `inferType` pass over every def body and were the source of OOM
  blowups on heavy projects. They are gone — the source slice supersedes them.

  Only follows project-local dependencies. External library declarations
  (Mathlib/Init/Std) are excluded from deps. Auto-generated sub-declarations are
  skipped; duplicate declarations observed through multiple modules are emitted once.
-/
import Lean
...
/-- Constants referenced in a `def`'s VALUE, dropping those whose own declaration
    is a theorem/lemma (proof terms). Cheap: the kernel's precomputed
    `getUsedConstants` plus one `env.find?` per referenced constant — no `MetaM`
    and no per-node `inferType` (the elaboration that made the old prop-erasure
    export OOM). Preserves the prior "exclude proof dependencies" intent: a def's
    elaborated value can embed `by` proofs that reference project lemmas, and those
    lemmas don't belong in the def's type-level dependency set. -/
def getDataValueConstants (env : Environment) (ci : ConstantInfo) : Array Name :=
  (getValueConstants ci).filter fun c =>
...
/-- Structure field metadata WITHOUT pretty-printing the field type: the field
    name, its projection name, and whether it came from a parent (via `extends`).
    Returns `none` for non-structures; iterates `fieldNames` (declaration order)
    rather than `fieldInfo` (alphabetical).

    The audit-digest renderer needs `projName` to filter standalone projection
    decls; the field TYPE is shown by the structure's verbatim `source_text`
    slice, so re-deriving it via `ppExpr` (the old OOM-prone path) is dead weight. -/
def getStructureFieldsMeta (env : Environment) (name : Name) : Option (Array Json) :=
...
-- ============================================================================
-- Verbatim source slicing + scope capture (`source_text`, `decl_namespace`,
-- `module_opens`, `module_notations`, `module_variables`, `module_context`).
--
-- These are the digest's primary content: raw byte slices of the original `.lean`
-- file, taken after parsing each module with Lean's own frontend (so scoped
-- notation parses). Unlike an elaborated pretty-print, a verbatim slice round-trips
-- to compiling source — which is why the old `ppExpr`-rendered fields were dropped.
-- ============================================================================
...
unsafe def main : List String → IO Unit := fun args => do
  ...
  -- Collect all project declaration names (for dependency filtering).
  ...
  -- Helper: collect deps for a declaration (type + non-proof value + field/ctor types).
  let collectDeps := fun (name : Name) (cinfo : ConstantInfo) => do
    -- Type signature deps
    let mut allConsts := getTypeConstants cinfo
    -- Value deps for defs, dropping referenced theorems (proof terms) — cheap,
    -- no per-node `inferType` (see `getDataValueConstants`).
    match cinfo with
    | .defnInfo _ => allConsts := allConsts ++ getDataValueConstants env cinfo
    | _ => pure ()
  ...
  for i in [:mods.size] do
    ...
    for j in [:md.constNames.size] do
      ...
      -- Collect project deps (type + non-proof value + field/ctor types)
      ...
      -- Structure field metadata (name/projName/fromParent; no type pp)
      let fieldsJson := getStructureFieldsMeta env name
